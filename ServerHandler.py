...
class ProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def get_send_inject(self):
        if sets.IQUERY:
            self.send_connection_logger('+++send tracker+++\r\n')
            if self.phost and self.pport:
                self.send_connection_logger('Proxy - %s:%s\r\n' % (self.phost, self.pport))
            elif sets.ADMODE:
                self.send_connection_logger('Host - %s:%s\r\n' % (self.advhost, self.port))
            else:
                self.send_connection_logger('Server - %s:%s\r\n' % (self.host, self.port))
        ...
        if not self.https:
            ...
            if index >= 0:
                ...
                if self.get_data_splitter(data):
                    self.send_connection_logger('+++Response+++\r\n%s\r\n' % data)
                    self.send_connection_logger('\r\n')
    ...
    def send_connection_error(self, msg, page = True):
        ...
        self.send_connection_logger('+++error bos+++\r\n')
        self.send_connection_logger('%s: %s\r\n\r\n' % (str(code), message))
        if page:
